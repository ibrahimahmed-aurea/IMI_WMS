...
namespace Imi.CodeGenerators.WebServices.XML2CreateTable
{
  ...
  /// </summary>
  public class CreateCommentGenerator : GenericGenerator
  {
    ...
    public CreateCommentGenerator(MessageDefinition messageDef, Package packageDef) : base( messageDef, packageDef )
    {
    }
    ...
    public override void GetOutPut(TextWriter writer) 
    {
      ...
      foreach(StructureType structure in messageDef.structures) 
      {
        ...
        //
        //  Table comments section, table comment is fetched from
        //  the structure document
        //
        if((structure.comment != null) && (structure.comment.Length > 0)) 
        {
        ...
        foreach(ParameterType p in parameterList) 
        {
          if(p.column != null) 
          {
            ...
            s.Append(p.name);
            s.Append(" is\r\n");
            s.Append(GetCommentString( GetComment( structure, p ) ));
            s.Append(";\r\n\r\n");
          }
        }
