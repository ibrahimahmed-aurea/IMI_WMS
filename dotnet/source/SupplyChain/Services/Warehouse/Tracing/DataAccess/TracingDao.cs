...
using Imi.SupplyChain.Warehouse.Tracing.DataAccess.Translators;
        ...
        public ModifyInterfaceTracingResult ModifyInterfaceTracing(ModifyInterfaceTracingParameters parameters)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                using (IDbConnection connection = new DbConnection(ConnectionString))
                {
                    ...
                    using (IDbCommand command = connection.CreateCommand())
                    {
                        command.CommandText = "SYSTEM_CONFIG.MODIFY_LOGG";
                        ((OracleCommand)command).BindByName = true;
        ...
        public IList<CheckInterfaceTracingResult> CheckInterfaceTracing(CheckInterfaceTracingParameters parameters)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                using (IDbConnection connection = new DbConnection(ConnectionString))
                {
                    ...
                    using (IDbCommand command = connection.CreateCommand())
                    {
                        command.CommandText = StatementCache.Instance.GetCachedStatement("Imi.SupplyChain.Warehouse.Tracing.DataAccess.Queries.CheckLogg.sql");
                        ((OracleCommand)command).BindByName = true;
        ...
        public IList<GetServerInformationResult> GetServerInformation(GetServerInformationParameters parameters)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                using (IDbConnection connection = new DbConnection(ConnectionString))
                {
                    ...
                    using (IDbCommand command = connection.CreateCommand())
                    {
                        command.CommandText = StatementCache.Instance.GetCachedStatement("Imi.SupplyChain.Warehouse.Tracing.DataAccess.Queries.GetServerInformation.sql");
                        ((OracleCommand)command).BindByName = true;
